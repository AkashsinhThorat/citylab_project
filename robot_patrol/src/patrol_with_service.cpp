...
public:
  Patrol()
      : rclcpp::Node("patrol_with_service_node"),
        in_flight_(false), obstacle_threshold_m_{0.35f}, linear_speed_{0.10f} {
    ...
    // QoS: LaserScan is sensor data (low-latency, best-effort) & just keepthe
    // latest scan
    auto laser_qos = rclcpp::SensorDataQoS().keep_last(1);
...
private:
  ...
  laser_callback(const sensor_msgs::msg::LaserScan::ConstSharedPtr &scan_msg) {
    ...
    if (front_min < obstacle_threshold_m_) {
      // 2) Obstacle detected: call the /direction_service to decide the further
      // movement

      // Don’t send the request if the service isn’t up yet
  ...
  void timer_callback() {
    ...
    RCLCPP_DEBUG_THROTTLE(get_logger(), *get_clock(), 2000,
                          "cmd: vx=%.2f, wz=%.2f", cmd.linear.x, cmd.angular.z);
  }

  // Called when the service replies
  void response_callback(rclcpp::Client<GetDirection>::SharedFuture future) {
    auto response = future.get();
    std::string decision_response = response->direction;
    RCLCPP_INFO(this->get_logger(), "Response: %s", decision_response.c_str());

    cmd.linear.x = linear_speed_;
    if (decision_response == "Move forward") {
      cmd.angular.z = 0.0;
    } else if (decision_response == "Turn left") {
      cmd.angular.z = 0.5;
    } else if (decision_response == "Turn right") {
      cmd.angular.z = -0.5;
    } else {
      RCLCPP_WARN(this->get_logger(),
                  "Service Response INVALID!, retaining previous state");
    }
  ...
  std::atomic<bool> in_flight_; // Simple back-pressure flag
  int control_period_ms_;

  geometry_msgs::msg::Twist cmd; // cmd to publish
